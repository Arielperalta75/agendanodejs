...
//Verificar si existe el usuario ARIEL
Router.get('/ariel', function(req, res) {
  Usuarios.find({user: req.query.user}).count({}, function(err, count) { //Verificar si existe el usuario ARIEL
    if(count>0){ //Si el registro es mayor a 0
        res.send("Utilice los siguientes datos: </br>usuario: ariel | password:12345 </br>usuario: juan | password:00000") //Mostrra mensaje con los datos de los usuarios predeterminados
    }else{
